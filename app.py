...
HOG_model = joblib.load("HOG_model.pkl")
...
st.title("Butterfly Tumor Image Classification App")
# For newline
st.write('\n')
...
if st.sidebar.button("Click Here to Classify"):
    ...
    if uploaded_file is None:
        ...
        with st.spinner('Classifying ...'):
            ...
            prediction = predict(HOG_model["w"], GBM_knn_clf_model["b"], my_image)
            #prediction = GBM_knn_clf_model.predict(my_image)
            time.sleep(2)
            st.success('Done!')
        ...
        # Classify PCNSL being present in the picture if prediction > 0.5

        if prediction > 0.5:

            st.sidebar.write("It's a 'PCNSL' picture.", '\n')

            st.sidebar.write('**Probability: **', probability, '%')


        else:
            st.sidebar.write(" It's a 'GBM' picture ", '\n')
